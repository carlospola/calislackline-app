...
&#x20; - \*\*⚠️ Learning operativo — stale della lista template nell'admin:\*\* modifiche ai `coach\_rules` dei TEMPLATE via Supabase Table Editor NON aggiornano la lista template tenuta in memoria nell'admin; il bottone "Prova" usa la copia STANTIA. Fare `Ctrl+Shift+R` (hard refresh) dopo ogni edit in Table Editor PRIMA di testare con "Prova". Il motore `coach\_prompt\_*` in `settings` è invece letto server-side da `chat.js`, FRESCO ad ogni richiesta: nessun refresh necessario.
...
\- modal #templateFormModal (tplName/tplType/tplRules/tplCsv/tplPrompt — i campi compilabili NON hanno più placeholder di esempio, commit `1410bd5`; solo il nome è obbligatorio, `coach\_rules`/`tplRules` opzionale)

\- per card: "Modifica" / "Elimina" / "Assegna" (#assignModal + #assignAthleteSelect -> confirmAssign; \*\*idempotente\*\*: guardia `assignInFlight` + bottone disabilitato durante la chiamata, commit `1410bd5`)
...
\- \*\*✅ Progressi v2 (25/06):\*\* Epley ora RIR-adjusted: `e1RM = peso*(1+(reps+rir)/30)`, `rirEff=0` se rir null (sostituisce `reps/30` del Batch 1; unica occorrenza `sessionMaxE1rm`). Titoli grafici tab Esercizio estesi per-ramo in `progress.js` ("Massimale stimato per sessione (kg)" / "Tonnellaggio per sessione (kg)" / "Media reps a serie (per sessione)" / "Reps totali per sessione"); descrizioni dinamiche nei nuovi div `pChartRepsDesc`/`pChartTotalRepsDesc` (via `esc()` + span formula `#8fb0c0`; isometrico = vuota). NUOVO grafico tab Esercizio "Margine al cedimento (RIR medio)" (canvas `chartRir`, var `chartRirInst`, media RIR per sessione su `rir != null`, barre 0-10). `renderSetDetailChart` (canvas `chartSets`): multi-metrica con assi `y`/`yE`/`yEffort`, controlli DOM, cap 60, no persistenza. \*\*Overview:\*\* 3 stat-card nuove (`ovStatReps`/`ovStatRir`/`ovStatAvgSets`); 4 grafici nuovi (`chartOvSets`/`chartOvTotReps`/`chartOvVol` SOLO `weight>0`/`chartOvRir` barre 0-10), var `chartOvSetsInst`/`chartOvTotRepsInst`/`chartOvVolInst`/`chartOvRirInst`; relabel torta e RPE. `var totalVolume` resta morta (residuo Batch 1).
