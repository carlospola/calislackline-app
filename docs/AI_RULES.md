...
&#x20; - \*\*Leva-RIR degli skill (MUP) — PARCHEGGIATA al "prompt unico per-esercizio" (vedi TASKS 3B):\*\* i `coach\_rules` MUP sono a v3 (RIR 0/1/2/3 = a target niente leva; solo RIR ≥4 fa scattare "riduci assistenza"), ma il modello NON onora la soglia sul bordo RIR=3. \*\*Lezione load-bearing:\*\* l'aderenza del modello a soglie RIR NUMERICHE dentro un prompt lungo è inaffidabile sul bordo → preferire un TRIGGER DETERMINISTICO (`reps > tetto`) al giudizio RIR del modello. Da risolvere nel prompt unico.
