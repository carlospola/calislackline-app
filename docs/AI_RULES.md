...
&#x20; - \*\*⚠️ CORREZIONE COACHING DA RAFFORZARE (regressione vista su MUP, Jumping Muscle-Up 5 su 3-5):\*\* il motore ricade ancora nell'errore "tetto del range = sforato". Regola da ribadire (Table Editor, no deploy): reps NEL range estremi inclusi = A TARGET; sforato SOLO se reps > tetto. Inoltre i `coach\_rules` MUP hanno una soglia RIR ambigua ("RIR≥2-3"): chiarire → \*\*RIR<3 = ok\*\* (a target), SOLO \*\*RIR≥3\*\* fa scattare la leva "riduci assistenza". Vedi TASKS (fix coaching range/RIR).

&#x20; - \*\*Task PROMPT UNICO (orizzonte, vedi TASKS):\*\* unificare i delta `coach\_prompt\_gym`/`coach\_prompt\_bodyweight` in un solo `coach\_prompt\_global` che applica la leva PER-ESERCIZIO leggendo la colonna `peso` (weighted → carico; corpo libero → variante/leva/tempo). Fattibile SOLO via `settings` (Table Editor): delta svuotati, `chat.js` e Leva 2 caching intatti. Renderebbe `session\_type` del tutto rimovibile dal motore. Dipende dalla colonna `peso` popolata su tutti i programmi.
